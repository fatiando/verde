...
results are equivalent to using :class:`verde.ScipyGridder` with ``method='cubic'`` but
the interpolation is usually slower. The advantage of using :class:`verde.Spline` is
that you can assign weights to the data to incorporate the data uncertainties or
variance into the gridding.

In this example, we'll use :class:`verde.BlockMean` to decimate the data because it can
calculate weights based on the data uncertainty from input data and pass it along to the
spline.
"""
