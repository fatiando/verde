...
class BaseGridder(BaseEstimator):
    ...
    * Estimated parameters should be stored as attributes with names ending in
    ...
    def predict_onto_grid(
        self, grid, data_names=None, dims=None, extra_coords=None, projection=None
    ):
        """
        Interpolate data on a preexisting regular grid

        Make predictions on a preexisting regular grid passed as
        a :class:`xarray.Dataset` and store them as a new data array inside
        it.

        Parameters
        ----------
        grid : :class:`xarray.Dataset`
            Dataset containing at least northing and easting coordinates
            where the predictions will be carried out.
        data_names : list or None
            The name(s) of the data variables used to store the predictions on
            the grid. Defaults to
            ``['scalars']`` for scalar data,
            ``['east_component', 'north_component']`` for 2D vector data, and
            ``['east_component', 'north_component', 'vertical_component']`` for
            3D vector data.
        dims : tuple
            The names of the northing and easting data dimensions,
            respectively, in the grid. Default is determined from the
            ``dims`` attribute of the class. Must be defined in the following
            order: northing dimension, easting dimension.
            **NOTE: This is an exception to the "easting" then
            "northing" pattern but is required for compatibility with xarray.**
        extra_coords : list or None
            Names of the additional coordinates present in the grid that should
            be passed to the ``predict`` method. By additional coordinate we
            mean any extra coordinate besides the northing and the easting
            ones. The additional coordinates will be passed in the same order
            that they appear on the list. If None, only the easting and
            northing coordinates will be used to predict. Default None.
        projection : callable or None
            If not None, then should be a callable object
            ``projection(easting, northing) -> (proj_easting, proj_northing)``
            that takes in easting and northing coordinate arrays and returns
        ...
        if extra_coords:
            coordinates += tuple(grid[coord].values for coord in extra_coords)
        ...
        # Get proper data names
        data_names = get_data_names(data, data_names)
        for data_name in data_names:
            # Check if data_name don't already exist on the grid
            if data_name in grid:
                raise ValueError(
                    "Data '{}' already exists on the passed grid. ".format(data_name)
                    + "Use a different name for the interpolated data."
                )
        # Append interpolated data to grid
        attrs = {"metadata": "Generated by {}".format(repr(self))}
        for name, value in zip(data_names, data):
            grid[name] = (dims, value, attrs)
